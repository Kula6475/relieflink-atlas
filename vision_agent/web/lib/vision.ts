...
export type VisionResult = {
  ...
  countMethod: "claude_two_pass" | "detector_fallback";
  countNote: string;
  mode: "cloud";
};
...
function parseClaudeCount(text: string): ClaudeCount {
  ...
  const counts = Object.fromEntries(
    CATEGORIES.map((category) => [
      category,
      Math.max(0, Math.round(Number(parsed.counts?.[category] ?? 0))),
    ]),
  ) as ClaudeCount["counts"];
  return { counts, notes: parsed.notes ?? "" };
}

async function countWithClaude(
  imageDataUrl: string,
): Promise<{ count: ClaudeCount; confidence: number }> {
  const apiKey = setting("ANTHROPIC_API_KEY");
  if (!apiKey)
    throw new Error("ANTHROPIC_API_KEY is not configured for Claude counting");
  ...
  async function pass(independent: boolean) {
      ...
      messages: [
        {
          ...
          content: [
            ...
            {
              type: "text",
              text: `${CLAUDE_COUNT_PROMPT}${independent ? " Perform an independent recount; do not copy a prior answer." : ""}`,
            },
          ],
        },
      ],
    ...
    const extractionResponse = await client.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `Extract the final count from this visual record. Return only the schema. Use the running tally, count partially visible items identified by rims/lids/edges, and preserve uncertainty in notes.\n\n${reasoning}`,
        },
      ],
    });
    const count = parseClaudeCount(
    ...
    return {
      counts: count.counts,
      notes: [count.notes, reasoning].filter(Boolean).join(" "),
    };
  }
  const [first, second] = await Promise.all([pass(false), pass(true)]);
  const deltas = CATEGORIES.map((category) =>
    Math.abs(first.counts[category] - second.counts[category]),
  );
  const confidence = deltas.length
    ? deltas.reduce(
        (sum, delta, index) =>
          sum +
          Math.max(
            ...
            1 -
              delta /
                Math.max(
                  first.counts[CATEGORIES[index]],
                  second.counts[CATEGORIES[index]],
                  1,
                ),
          ),
        0,
  ...
  const counts = Object.fromEntries(
    CATEGORIES.map((category) => [
      category,
      Math.round((first.counts[category] + second.counts[category]) / 2),
    ]),
  ) as ClaudeCount["counts"];
  const disagreement = CATEGORIES.filter(
    (category) => first.counts[category] !== second.counts[category],
  )
    .map(
      (category) =>
        `${category} differed (${first.counts[category]} vs ${second.counts[category]})`,
    )
    .join("; ");
  return {
    count: {
      ...
      notes: [
        first.notes,
        second.notes,
        disagreement && `Independent-pass disagreement: ${disagreement}`,
      ]
        .filter(Boolean)
        .join(" "),
    },
...
async function runRoboflowRequest(
  base64: string,
  apiKey: string,
  model: string,
) {
  const response = await fetch(
    `https://serverless.roboflow.com/${model
      .split("/")
      .map((part) => encodeURIComponent(part))
      .join(
        "/",
      )}?api_key=${encodeURIComponent(apiKey)}&confidence=${encodeURIComponent(setting("YOLO_CONFIDENCE") ?? "20")}&overlap=30`,
    {
      method: "POST",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Roboflow inference for ${model} failed (${response.status}): ${detail}`,
    );
  }
...
function filterPredictions(result: RoboflowResponse): Detection[] {
  ...
  return (result.predictions ?? [])
    ...
    .filter(
      (item) =>
        countClasses.length === 0 ||
        countClasses.includes(item.className.toLowerCase()),
    );
}
    ...
    if (
      !kept.some(
        (existing) =>
          ...
          intersectionOverUnion(existing, detection) >= threshold,
      )
    ) {
      kept.push(detection);
    }
...
function filterTileArtifacts(
  ...
  const edgeMargin = Math.max(
    0.01,
    Math.min(0.15, Number(setting("YOLO_TILE_EDGE_MARGIN") ?? "0.04")),
  );
  const maxArea = Math.max(
    0.2,
    Math.min(1, Number(setting("YOLO_TILE_MAX_BOX_AREA") ?? "0.55")),
  );
  return detections.filter((detection) => {
    ...
    const clipped =
      left <= tileWidth * edgeMargin ||
      top <= tileHeight * edgeMargin ||
      right >= tileWidth * (1 - edgeMargin) ||
      bottom >= tileHeight * (1 - edgeMargin);
    const oversized =
      detection.width * detection.height >= tileWidth * tileHeight * maxArea;
...
async function runRoboflow(
  bytes: Buffer,
): Promise<{ predictions: Detection[]; width: number; height: number }> {
  ...
  const grid = Math.max(
    1,
    Math.min(3, Math.round(Number(setting("YOLO_TILE_GRID") ?? "3"))),
  );
  if (grid === 1) {
    const result = await runRoboflowRequest(
  ...
  const overlap = Math.min(
    0.45,
    Math.max(0.1, Number(setting("YOLO_TILE_OVERLAP") ?? "0.25")),
  );
  const tileWidth = Math.min(
    width,
    Math.ceil((width * (1 + overlap)) / (grid + overlap)),
  );
  const tileHeight = Math.min(
    height,
    Math.ceil((height * (1 + overlap)) / (grid + overlap)),
  );
  const tiles = Array.from({ length: grid * grid }, (_, index) => {
    const column = index % grid;
    const row = Math.floor(index / grid);
    const left =
      grid === 1 ? 0 : Math.round(((width - tileWidth) * column) / (grid - 1));
    const top =
      grid === 1 ? 0 : Math.round(((height - tileHeight) * row) / (grid - 1));
    return { left, top };
  });
  const tileResults = await Promise.all(
    tiles.map(async ({ left, top }) => {
      const tileBytes = await sharp(bytes)
        .extract({ left, top, width: tileWidth, height: tileHeight })
        .jpeg({ quality: 90 })
        .toBuffer();
      const result = await runRoboflowRequest(
        tileBytes.toString("base64"),
        apiKey,
        model,
      );
      const resultWidth = result.image?.width ?? tileWidth;
      const resultHeight = result.image?.height ?? tileHeight;
      const tileDetections = filterTileArtifacts(
        filterPredictions(result),
        tileWidth,
        tileHeight,
      );
...
async function runLocalCannedGoods(
  bytes: Buffer,
): Promise<{ predictions: Detection[]; width: number; height: number }> {
  ...
  for (const candidate of candidates.sort(
    ...
    if (!kept.some((existing) => boxIoU(existing, candidate) >= 0.45))
      kept.push(candidate);
  }
...
async function runDetector(bytes: Buffer) {
  if (existsSync(resolve(process.cwd(), "models/canned_goods.onnx"))) {
    try {
      ...
      if (setting("ROBOFLOW_API_KEY") && setting("YOLO_MODEL_ID"))
      ...
      messages: [
        {
          ...
          content: [
            {
              type: "text",
              text: "Identify the food product and broad category. Do not count objects. Return JSON with product, category, packaging, confidence from 0 to 1, and uncertainty (string or null).",
            },
...
export async function analyzeStillImage(input: {
  ...
  const localModel = existsSync(
    resolve(process.cwd(), "models/canned_goods.onnx"),
  );
  ...
  const claude = await countWithClaude(imageDataUrl).catch(() => null);
  const llm = await classifyWithVision(imageDataUrl);
  const totalClaudeCount = claude
    ? Object.values(claude.count.counts).reduce((sum, count) => sum + count, 0)
    : 0;
  const detectorCount = detections.length;
  const usedDetectorFallback =
    !claude || (totalClaudeCount === 0 && detectorCount > 0);
  ...
  const classification = llm
    ...
    : {
        ...
        confidence: 0,
        uncertainty:
          "Cloud product classification was unavailable. Confirm the product and category before approval.",
      };
  const classSet = new Set(
    detections.map((item) => item.className.toLowerCase()),
  );
  const disagreement =
    classSet.size > 1
  ...
  return {
    ...
    classification,
    disagreement,
    countMethod: usedDetectorFallback ? "detector_fallback" : "claude_two_pass",
    countNote: usedDetectorFallback
      ? `Cloud counting was unavailable or returned no count, so the UI shows ${detectorCount} detector packages as a conservative fallback. Confirm or correct the quantity before approval. ${claude?.count.notes || ""}`
