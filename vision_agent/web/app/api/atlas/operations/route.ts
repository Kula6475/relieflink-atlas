...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [disruptions, inventory, history, network] = await Promise.all([
      ...
      sql()`SELECT category,sum(quantity)::float quantity FROM inventory_items WHERE site_id=${context.siteId} AND condition='good' AND archived_at IS NULL GROUP BY category`,
